...
class Experiment:
    ...
    def add_metrics(self, metrics):
        ...
        response = requests.post(
            f"{self.BASE_URL}/v1/llm/experiment",
            headers=headers,
            json=json_data,
            timeout=Experiment.TIMEOUT,
        )
        ...
        if status_code == 200:
            ...
            self.project_id = test_response.get("data").get("experiment").get("projectId")
            print(f"Metrics added successfully. Job ID: {self.job_id}")
            metric_names = [execution["metricName"] for execution in test_response["data"]["experiment"]["executions"]]
